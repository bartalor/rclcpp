...
# Generate compose.generated.yaml with include: entries for enabled plugins
# that contribute a compose.yaml. Must happen pre-build (initializeCommand).
HERE="$(cd "$(dirname "$0")" && pwd)"
# shellcheck source=plugins.sh
. "$HERE/plugins.sh"

"$HERE/uninstall-disabled-plugins.sh"
{
    echo "# Auto-generated by initialize-command.sh. Do not edit."
    echo "include:"
    for plugin in "${PLUGINS[@]}"; do
        if [ -f "$HERE/plugins/$plugin/compose.yaml" ]; then
            echo "  - plugins/$plugin/compose.yaml"
        fi
    done
} > "$HERE/compose.generated.yaml"
...
for plugin in "${PLUGINS[@]}"; do
    hook="$HERE/plugins/$plugin/initialize-command.sh"
    if [ -f "$hook" ]; then
        echo "plugin $plugin: running initialize-command.sh"
