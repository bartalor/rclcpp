# `windows-repro-27999.yml` — line-by-line research notes

This document explains every meaningful line in `.github/workflows/windows-repro-27999.yml`. For every value, flag, arg, and step it answers:
...
Citations like `windows-repro-27999.yml › "anchor"` or `windows-repro-27999.yml › <step name>` point to a search anchor in the workflow file — open the file and Ctrl-F. (We avoid line numbers because they drift on every edit and the surface area to maintain is too large.) Citations like `log:NNNN` refer to `/tmp/ci_windows_27999.log` (use `sed -n 'NNNNp' /tmp/ci_windows_27999.log` to look them up).
...
## Intentional deviations from #27999 (header comment of `windows-repro-27999.yml`)
...
- `group: windows-repro-27999` — single group name; all dispatches go through the same queue.
...
Brings the workflow repo into the runner workspace so `.github/windows-repro-27999.Dockerfile` is on disk for the next step, and so subsequent steps have a CWD with write access for the `workspace/` mount target.
...
- name: Wait for Docker
  shell: pwsh
  run: |
    for ($i = 1; $i -le 60; $i++) { docker info ... ; if (...) { exit 0 } ... }
```

Workaround for [actions/runner-images#13729](https://github.com/actions/runner-images/issues/13729): Docker Engine v29+ on `windows-2022` randomly fails to start at runner boot due to a Hyper-V virtual-switch race. Maintainer-confirmed open as of 2026-05-15, no ETA. First hit us in run `26512474496` (`failed to connect to the docker API at npipe:////./pipe/docker_engine`).

Shape mirrors [spiffe/spire's wait-for-docker-windows.ps1](https://github.com/spiffe/spire/blob/cb7073c250ef6eb568b837e4448cc58a30450067/.github/workflows/scripts/wait-for-docker-windows.ps1) — the proven workaround referenced in the upstream issue:
- Readiness probe is `docker info` (round-trips the daemon), not `Test-Path` on the named pipe (pipe existence ≠ daemon ready).
- On the first failed attempt, enumerate every `*docker*` service and `Start-Service` any that aren't running (covers `docker` and `com.docker.service`).
- Poll up to 60 × 2s = 120s before erroring out.

### Dockerfile: `.github/windows-repro-27999.Dockerfile`
...
- Locally reproducible: `docker build -f .github/windows-repro-27999.Dockerfile --build-arg PIXI_VERSION=v0.41.0 ...` runs without the workflow.

Pins are passed via `--build-arg` from the workflow's `env:` block so the workflow remains the single source of truth.

Going line by line through `.github/windows-repro-27999.Dockerfile`:
...
- `ENV PIXI_VERSION=${PIXI_VERSION}` / `ENV PIXI_ZIP_SHA256=${PIXI_ZIP_SHA256}` — **promotion of build-args to env vars**. Per [Docker docs › Environment replacement](https://docs.docker.com/reference/dockerfile/#environment-replacement), the builder does NOT substitute `${VAR}` inside `RUN` lines — substitution there is the shell's job. ENV makes them visible to PowerShell as `$env:VAR`. The missing `ENV` step was the root cause of run `26445844372`'s `Step 11/22` failure (`irm : Not Found` on `…/download/${PIXI_VERSION}/…`).
...
- `RUN powershell -noexit "irm .../pixi-...$env:PIXI_VERSION/pixi-...zip ..."` — mirrors `log:170` (`Step 12/32`). Uses `$env:PIXI_VERSION` (PowerShell env-var syntax) since the Dockerfile builder does not substitute `${VAR}` inside `RUN` — see the `ENV PIXI_VERSION=...` line above.
- `RUN powershell -noexit "if ((get-filehash ...).hash -ne $env:PIXI_ZIP_SHA256) { exit 1 }"` — mirrors `log:173` (`Step 13/32`). SHA-256 integrity check; same `$env:VAR` reason as above.
...
- `-f .github/windows-repro-27999.Dockerfile` — explicit path to the checked-in Dockerfile.
