...
Brings the workflow repo into the runner workspace so subsequent steps can write files alongside it. The workflow itself doesn't use the checkout for source code (the source comes from `vcs import` inside the container via `run_ros2_batch.py`); it just needs a CWD with write access.

### Step: Write Dockerfile (`windows-repro.yml:39-86`)

Emits a Dockerfile to disk that mirrors #27999's Steps 1-24 minus the deviations described above. Each block of the heredoc corresponds to a Dockerfile line from the reference:

- `# escape=`` ` — Dockerfile escape directive. Required to use backtick as the line-continuation character in PowerShell-friendly RUN lines (matching `\` would conflict with Windows path separators).
- `ARG ROS_DISTRO=rolling` — mirrors `log:146` `Step 3/32 : ARG ROS_DISTRO=rolling`.
...
- `ARG ROS_DISTRO` — re-declares the build arg after `FROM` so it's available in this stage. Mirrors `log:149` `Step 5/32 : ARG ROS_DISTRO`.
- `ARG PIXI_TOML_URL=...${env:PIXI_TOML_SHA}/pixi.toml` — **deliberate divergence from `log:152`**: reference uses `refs/heads/${ROS_DISTRO}`, we substitute the frozen `PIXI_TOML_SHA` so the env is reproducible after `ros2/ros2@rolling` advances.
...
- `RUN powershell ... vs_buildtools.exe ...` (2022 only) — mirrors `log:164` (`Step 10/32`). **Deviation 2**: we skip `log:158-161` (`Steps 8-9`, VS 2019 BuildTools download + install).
- `RUN vs_buildtools_2022.exe --quiet --wait --norestart --add ...` — verbatim mirror of `log:167` (`Step 11/32`), same component list. The component set matters because:
...
- `RUN powershell -noexit irm ...pixi-...zip` — mirrors `log:170` (`Step 12/32`). Downloads pixi `v0.41.0` (the value of `${env:PIXI_VERSION}` is interpolated in by PowerShell when the Dockerfile is written).
- `RUN powershell -noexit "if (...get-filehash...)..."` — mirrors `log:173` (`Step 13/32`). SHA-256 integrity check.
- `RUN powershell -noexit Expand-Archive ...` — mirrors `log:176` (`Step 14/32`). Unzips pixi into `%USERPROFILE%\.pixi\bin`.
- `RUN powershell -noexit "$bindir = ... $pathkey.SetValue('PATH', ...)"` — mirrors `log:179` (`Step 15/32`). Prepends pixi's bin dir to the persistent registry PATH.
- `WORKDIR C:\pixi_ws` — mirrors `log:185` (`Step 17/32`). The pixi project root for the env install.
- `ADD ${PIXI_TOML_URL} pixi.toml` — mirrors `log:191` (`Step 19/32`). Downloads our pinned `pixi.toml`. Important: we **skip** `log:188` (`Step 18/32`, the HTTPS-protocol check on `$PIXI_TOML_URL`) because our URL is fixed at workflow-write time, not user-controlled — the check is dead weight here.
...
`Out-File -Encoding ascii -FilePath Dockerfile` — writes the heredoc to disk as ASCII (docker build doesn't accept BOM-prefixed UTF-8 from PowerShell's default).

`Get-Content Dockerfile` — dumps the file to the GHA log for debug parity.

### Step: docker build image (`windows-repro.yml:88-90`)
...
- `shell: cmd` — `%VAR%` syntax for env var interpolation is cmd.exe; pwsh would need `$env:VAR`. Either works; we picked cmd to keep the docker command line in a single shell mode across build and run.
- `--isolation=process` — matches `log:308` `docker run --isolation=process`. Note: at build time, `--isolation=process` requires the base image kernel to match the host kernel, which is exactly why Deviation 1 bumped the base to `:ltsc2022`.
- `-t %IMAGE_TAG%` — tag the built image so the subsequent `docker run` references it by name.
- `.` — build context = CWD (where the Dockerfile was just written).

### Step: Clone ros2/ci into workspace (`windows-repro.yml:92-99`)
...
### Step: docker run (`windows-repro.yml:101-109`)
...
### Step: Upload test results (`windows-repro.yml:111-119`)
...
3. **`retention-days: 7`** on artifact upload (`windows-repro.yml:119`). No reference. Local convention.
